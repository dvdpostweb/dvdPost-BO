...
Public Class frmEMailCustomers_RefactFev2008
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(frmEMailCustomers_RefactFev2008))
...
#End Region
    ...
    Dim intmessages_id As Integer = 221
    ...
    Public Overrides Sub LoadDataSet()
        ...
        SQLTxt1 = "SELECT C.customers_id , customers_gender, customers_firstname ,customers_lastname , C.customers_email_address , " & _
                  " customers_abo , customers_abo_type, customers_abo_dvd_credit, customers_abo_dvd_norm, customers_abo_dvd_adult, " & _
                  " customers_abo_dvd_home_norm, customers_abo_dvd_home_adult , ac.activation_group, " & _
          " if (ac.activation_code_creation_date = '0000-00-00 00:00:00', Null, ac.activation_code_creation_date) as activation_code_creation_date " & _
          " , customers_next_abo_type, count(mail_messages_id) as already_sent,  C.selected_for_email, C.site,  " & _
          " if (customers_abo_validityto = '0000-00-00', Null, customers_abo_validityto) as customers_abo_validityto,  " & _
          " if (customers_dob = '0000-00-00', Null, customers_dob) as customers_dob, customers_language, activation_discount_code_id  " & _
          " , payerror.correct_amount, payerror.bad_amount, payerror.transaction_date, payerror.payment_type, payerror.payment_id  " & _
          " FROM payment_error_recovery payerror left join customers C on payerror.customers_id = C.customers_id" & _
          " LEFT JOIN activation_code ac on ac.customers_id = C.customers_id " & _
          " LEFT JOIN mail_messages_sent_history mh on mh.customers_id = C.customers_id and mail_messages_id = " & intmessages_id & " " & _
          " where payment_error_recovery_id > 2975"
        ...
        Try
            ...
            OrderByClause = " GROUP BY C.customers_id"
    ...
    Private Sub btnUser1_ItemClick(ByVal sender As System.Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnUser1.ItemClick
        ...
        For _i = 0 To GridView1.RowCount - 1
            ...
            For Each _MailRow In objDS.Tables("mail_messages").Rows
                ...
                'Correction Amount
                strMessage = Replace(strMessage, "$$$bad_amount$$$", _CustRow.Item("bad_amount"))
                strMessage = Replace(strMessage, "$$$correct_amount$$$", _CustRow.Item("correct_amount"))
                strMessage = Replace(strMessage, "$$$diff_amount$$$", Math.Round(_CustRow.Item("correct_amount") - _CustRow.Item("bad_amount"), 2))
                strMessage = Replace(strMessage, "$$$transaction_date$$$", _CustRow.Item("transaction_date"))
