...
#End Region
    ...
    'Private Sub AttributionDVDCustomer(ByRef CustomerStat As DvdPostData.Customer_stat, ByVal maxDVD As Integer, ByVal compensation As Integer, ByVal dt As DataTable, ByVal customers_id As Integer)
    ...
    '        dr = dt.Rows(index - 1)
    '        AssignDVD(cptNoCompensation <= 0, dr("wl_id"), dr("products_id"), customers_id, dr("products_dvdid"), DvdPostData.ClsCustomersData.OrdersType.ABOPROCESS)
    '        LockDVDProduct(dr("products_id"))
    ...
    Private Function FilterAvailableDVD(ByRef typeDvdCustomer As DvdPostData.Customer_TypeDvd, ByVal custRow As DataRowView, ByVal PRoductType As String) As DataTable
        ...
        WL_ViewSeries = New DataView(DS.Tables("wishlist"), "wishlist_type = '" & PRoductType & "'", "imdb_id,products_series_id, products_series_number,priority,date_added", DataViewRowState.CurrentRows)
        WL_Viewquantity = New DataView(DS.Tables("wishlist"), "wishlist_type = '" & PRoductType & "'", "priority,theme desc,qty_in_stock " & sortDisponibilite, DataViewRowState.CurrentRows)
        ...
        If WL_ViewSeries.Count > 0 Then
            ChooseDVDinWishlistInStock(custRow, PRoductType, WL_ViewSeries, WL_Viewquantity)
            ' dv_DVDWishlist = New DataView(DS.Tables("customers_" & PRoductType), " ", "priority,imdb_id,products_series_id, products_series_number,quantity desc", DataViewRowState.CurrentRows)
        End If
        '  Return dv_DVDWishlist
        Return DS.Tables("customers_" & PRoductType)
    End Function
    ...
    Private Sub aboprocess_main(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        CustView = New DataView(DS.Tables("customers"), "", "IsCompensation desc,customers_abo_rank desc,time_average asc, customers_id ", DataViewRowState.CurrentRows)
    ...
    Private Sub aboprocess(ByVal aboprocess_id As Integer, ByVal theme_event_id As Integer)
        ...
        For Each custRow In CustView
            Try
                ...
                customerStat = New DvdPostData.Customer_stat(customers_id, custRow("sleep"))
                ...
                If processCustom Then
                    ...
                    If (customerStat.dvd_norm.dvd_abo + customerStat.dvd_norm.dvd_compensation > 0) Then
                        ...
                        dt_DVDWishlistNormFound = FilterAvailableDVD(customerStat.dvd_norm, custRow, _PRoductType)
                        'dt_DVDWishlistNormFound IsNot Nothing AndAlso
                        If dt_DVDWishlistNormFound.Rows.Count > 0 Then
                            ' dv_DVDWishlistNorm.Count = 0 => dvdNotFound
                            cptStockDVDNormFound = dt_DVDWishlistNormFound.Rows.Count
                    ...
                    If (customerStat.dvd_adult.dvd_abo + customerStat.dvd_adult.dvd_compensation > 0) Then
                        ...
                        dt_DVDWishlistAdultFound = FilterAvailableDVD(customerStat.dvd_adult, custRow, _PRoductType)
                        ' dt_DVDWishlistAdultFound IsNot Nothing AndAlso
                        If dt_DVDWishlistAdultFound.Rows.Count > 0 Then
                            cptStockDVDAdultFound = dt_DVDWishlistAdultFound.Rows.Count
                    ...
                    AttributionDVDCustomerEquilibre(customerStat, DVDNormCalculated, NormCompensation, dt_DVDWishlistNormFound, customers_id)
                    AttributionDVDCustomerEquilibre(customerStat, DVDAdultCalculated, AdultCompensation, dt_DVDWishlistAdultFound, customers_id)
                    ...
                    If (Not wishlistEmpty) Then
                        manageRank(customers_id, IsCustomerServed)
                    End If

                    If IsCustomerServed Then
                        position_current += 1
                        customerStat.position = position_current
                        ...
                        Or (cptStockDVDAdultFound < cptDVDAdultRequest And cptDVDAdultRequest > 0) Then
                            ...
                            customerStat.reasonNorm = DvdPostData.Customer_stat.TypeReason.PARTIAL_SERVED
                        End If
                    Else
                        _AboStat.CptCustomerNoServed += 1

                        If (wishlistEmpty) Then
                            customerStat.reasonNorm = DvdPostData.Customer_stat.TypeReason.WISHEMPTY
                        ElseIf (cptStockDVDNormFound = 0 And cptDVDNormRequest > 0) Or (cptStockDVDAdultFound = 0 And cptDVDAdultRequest > 0) Then
                            ...
                            customerStat.reasonNorm = DvdPostData.Customer_stat.TypeReason.NOTFOUNDDVD
                            position_current += 1
                            customerStat.position = position_current
                        ...
                        ElseIf (cptDVDNormRequest > 0 And DVDNormCalculated = 0) Or (cptDVDAdultRequest > 0 And DVDAdultCalculated = 0) Then
                            ...
                            customerStat.reasonNorm = DvdPostData.Customer_stat.TypeReason.RANKTOOLOW
                        Else
            ...
            Catch ex As System.Exception
                ...
                _AboStat.CPTError += 1
            Finally
                DvdPostData.clsConnection.ExecuteNonQuery(customerStat.strUpdateStatCustomer(aboprocess_id))
    ...
    Private Function GetProductAvailable(ByVal product_id As Integer) As DataRowView
        ...
        'min_id = getMinNbr_rotation(_ProductRows)
        ...
        'If _ProductRows.Length = 0 Then
        '    Return Nothing
        'Else
        '    Return _ProductRows(0)
        'End If
    ...
    Private Function GetFirstDVDSerie(ByVal WLRow As DataRowView, ByVal wl_viewSeries As DataView) As DataView
        ...
        wl_viewFirst = New DataView(wl_viewSeries.Table, "imdb_id = " & WLRow("imdb_id"), "products_series_id,products_series_number", DataViewRowState.Unchanged)
        Return wl_viewFirst
    End Function
    Private Function IsSerieDVD(ByVal WLRow As DataRowView) As Boolean

        Return WLRow("imdb_id") > 0 AndAlso (Not WLRow("products_series_id") Is DBNull.Value) AndAlso (WLRow("products_series_id") <> 0)
    End Function
    ''' <summary>
    ...
    ''' <remarks></remarks>
    Private Sub ChooseDVDinWishlistInStock(ByVal custRow As DataRowView, ByVal PRoductType As String, ByVal wl_ViewSeries As DataView, ByVal wl_quantityDVD As DataView) '(ByVal _WLRows() As DataRowView)
        ' Dim WL_RowQuantityDVD As DataRowView
        ...
        Try
            '   While index < wl_quantityDVD.Count And _MaxDVDSendByCustomers > DS.Tables("customers_" & PRoductType).Rows.Count
            For Each WL_RowQuantityDVD As DataRowView In wl_quantityDVD
                ' max x dvd envoye 
                'If _MaxDVDSendByCustomers = DS.Tables("customers_" & PRoductType).Rows.Count Then
                '    Exit For
                'End If

                '  If wl_quantityDVD(index).Row.RowState = DataRowState.Unchanged Then
                '   WL_RowQuantityDVD = wl_quantityDVD(index)
                ' find dvd in stock 
                ...
                If IsSerieDVD(WL_RowQuantityDVD) Then
                    ' rechercher le 1er dvd de la serie dans la structure wl_ViewSeries  
                    ...
                    If _ProductRow IsNot Nothing Then 'A Product Row is found.
                        ...
                        AddDvdInTemporary(custRow, _
                                          ...
                                          _ProductRow("products_series_number"), 0)
                    ...
                    Else
                        ...
                        imdb_old = WL_RowQuantityDVD("imdb_id")
                ...
                Else ' not serie
                    If _ProductRow IsNot Nothing Then 'A Product Row is found.
                        ...
                        AddDvdInTemporary(custRow, _
                                         ...
                                         _ProductRow("imdb_id"), _
                                         _ProductRow("products_series_id"), _
                                         _ProductRow("products_series_number"), WL_RowQuantityDVD("qty_in_stock"))
