...
#End Region
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        _str = "date("
        Dim _SQLTXt As String = " SELECT * FROM envelope_labels_multishipment L LEFT join orders O on O.orders_id = L.orders_ID WHERE " & _
        ...
        BKGlobal.Cls1.FillDataSet(objDS.Tables("envelope_labels_multishipment"), _SQLTXt)
    ...
    Private Sub AddLabelRow(ByVal _row As DataRow, ByVal _DS As DataSet)
        ...
        _labelRow = _DS.Tables("envelope_labels_multishipment_det").Rows.Find(New Object() {_row.Item("multishipment_id")})
        If IsNothing(_labelRow) Then
            _labelRow = _DS.Tables("envelope_labels_multishipment_det").NewRow
            _labelRow.Item("multishipment_id") = _row.Item("multishipment_id")
            ...
            If _row.Item("delivery_country") = "Belgium" Then
                _labelRow.Item("delivery_country") = ""
            Else
                _labelRow.Item("delivery_country") = _row.Item("delivery_country")
            End If
            _labelRow.Item("delivery_postcode") = _row.Item("delivery_postcode")
            _labelRow.Item("lang_name") = _row.Item("lang_name")
            If _row.Item("secured_mail") > 0 Then
                _labelRow.Item("secured_mail") = "Secured Mail"
            Else
                _labelRow.Item("secured_mail") = ""
            End If
            _labelRow.Item("group_abc") = _row.Item("group_abc")
            _DS.Tables("envelope_labels_multishipment_det").Rows.Add(_labelRow)
        End If
        _labelRow.BeginEdit()
        _labelRow.Item(_row.Item("binome_xy") & "_orders_id") = _row.Item("orders_id")
        _labelRow.Item(_row.Item("binome_xy") & "_products_id") = _row.Item("products_id")
        _labelRow.Item(_row.Item("binome_xy") & "_dvd_id") = _row.Item("dvd_id")
        _labelRow.Item(_row.Item("binome_xy") & "_box_id") = _row.Item("box_id")
        _labelRow.Item(_row.Item("binome_xy") & "_box_dvd_id") = _row.Item("box_dvd_id")
        _labelRow.Item(_row.Item("binome_xy") & "_group_login_id") = _row.Item("group_id")
        _labelRow.Item(_row.Item("binome_xy") & "_ean128") = _row.Item("ean128")
        _labelRow.EndEdit()
    End Sub
