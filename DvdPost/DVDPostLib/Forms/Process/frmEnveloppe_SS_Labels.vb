...
#End Region
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        Else
            _SQLTXt = " SELECT * FROM envelope_labels L LEFT join orders O on O.orders_id = L.orders_ID WHERE " & _
                                    ...
                                    txtDate.DateTime.Year & "/" & txtDate.DateTime.Month & "/" & txtDate.DateTime.Day & "', '%Y/%m/%d')"
        End If
        DvdPostData.clsConnection.FillDataSet(objDS.Tables("envelope_labels"), _SQLTXt)
    ...
    Private Sub AddLabelRow(ByVal _row As DataRow, ByVal _DS As DataSet)
        ...
        _labelRow = _DS.Tables("envelope_labels_multishipment_det").Rows.Find(New Object() {_row("envelope_labels_id")})
        If IsNothing(_labelRow) Then
            _labelRow = _DS.Tables("envelope_labels_multishipment_det").NewRow
            _labelRow("multishipment_id") = _row("envelope_labels_id")
            ...
            _labelRow("lang_name") = BKGlobal.ExecuteFindFieldValue("name", "SELECT L.name FROM customers C left join languages L on C.customers_language = L.languages_id WHERE C.customers_id = " & _row("customers_id"))
            If _row("secured_mail") > 0 Then
                _labelRow("secured_mail") = "Secured Mail"
            Else
                _labelRow("secured_mail") = ""
            End If
            _labelRow("group_abc") = "a" '_row("group_abc")
            _DS.Tables("envelope_labels_multishipment_det").Rows.Add(_labelRow)
        End If
        _labelRow.BeginEdit()
        _labelRow("X_orders_id") = _row("orders_id")
        _labelRow("X_products_id") = _row("products_id")
        _labelRow("X_dvd_id") = _row("dvd_id")
        _labelRow("X_box_id") = _row("box_id")
        _labelRow("X_box_dvd_id") = _row("box_dvd_id")
        _labelRow("X_group_login_id") = _row("group_id")
        _labelRow("X_ean128") = _row("ean128")
        _labelRow.EndEdit()
    End Sub
