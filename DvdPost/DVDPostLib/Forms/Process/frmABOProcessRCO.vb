...
#Region " Windows Form Designer generated code "
    ...
    Friend WithEvents btnStartABO As DevExpress.XtraEditors.SimpleButton
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.btnStartABO = New DevExpress.XtraEditors.SimpleButton
        ...
        Me.XtraTabPage1.Controls.Add(Me.btnStartABO)
        ...
        'btnStartABO
        '
        Me.btnStartABO.Enabled = False
        Me.btnStartABO.Location = New System.Drawing.Point(648, 8)
        Me.btnStartABO.Name = "btnStartABO"
        Me.btnStartABO.Size = New System.Drawing.Size(168, 37)
        ToolTipTitleItem1.Text = "ABO Flow"
        ToolTipItem2.LeftIndent = 6
        SuperToolTip2.Items.Add(ToolTipTitleItem1)
        SuperToolTip2.Items.Add(ToolTipItem2)
        Me.btnStartABO.SuperTip = SuperToolTip2
        Me.btnStartABO.TabIndex = 36
        Me.btnStartABO.Text = "START (All Cust by ALL Cust)"
        '
        'XtraTabPage2
        '
...
#End Region
    ...
    Public Sub New(ByVal CurrentSessionInfo As BizzLib.clsSessionInfo)
        ...
        InitializeComponent()
    End Sub
    Private Sub frmABOProcessRCO1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    End Sub
    Private Sub SimpleButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnStartABO.Click
        aboprocess3(sender, e)
    End Sub
    ...
    End Sub
    Private Sub aboprocess3(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Do
            ...
            txtCustomersToProcess.EditValue = txtTotalCustomers.EditValue
            For Each _custRow In CustView 'DS.Tables("customers").Rows
                _IsCustInNeed = (_custRow.Item("NormToSend") <> 0 Or _custRow.Item("AdultToSend") <> 0)
                _IsCustomerServed = False
                _IsCompensation = False
                If _IsCustInNeed Then
                    'Do
                    If _custRow.Item("NormToSend") <> 0 Or _custRow.Item("AdultToSend") <> 0 _
                    Or _custRow.Item("NormCompensation") <> 0 Or _custRow.Item("AdultCompensation") <> 0 Then 'Exit Do
                        'Setting Products_Type and Compensation Flag
                        'Sending First the Compensation Norm and Adult, then standard wishlist Norm and Adult
                        If _custRow.Item("AdultToSend") > 0 Then
                            _PRoductType = "DVD_ADULT"
                            _IsCompensation = False
                        End If
                        If _custRow.Item("NormToSend") > 0 Then
                            _PRoductType = "DVD_NORM"
                            _IsCompensation = False
                        End If
                        If _custRow.Item("AdultCompensation") > 0 Then
                        ...
                        End If
                        _IsDVDFound = False
                        'Looping through the WishList of the Customer.
                        For _i = _iStart To _iEnd Step _iStep
                            _WLRowID = WLView.Find(New Object() {_custRow.Item("customers_id"), _PRoductType, _i})
                            If _WLRowID <> -1 Then
                                _WLRow = WLView(_WLRowID)
                                'Find the Product Info of the selected WishList Record - With INOUT = 1 (DVD is in stock)
                                _ProductRows = _ProductView.FindRows(New Object() {_WLRow.Item("product_id"), 1})
                                If _ProductRows.GetLength(0) > 0 Then 'A Product Row is found.
                                    _ProductRow = _ProductRows(0)
                        ...
                        Else
                            Select Case _PRoductType
                                'Customer is not served, cannot find anything in the wishist then set to ZERO remaining Qty
                                Case "DVD_NORM"
                                    _custRow.Item("NormCompensation") = 0
                                    _custRow.Item("NormToSend") = 0
                                    '_NormToSend = 0
                                Case "DVD_ADULT"
                                    _custRow.Item("AdultCompensation") = 0
                                    _custRow.Item("AdultToSend") = 0
                                    '_AdultToSend = 0
                            End Select
                        End If
                        DoEvents()
                    End If
                    'Loop
                    If _loop = 1 Then 'Changing Counter only during the first loop
                        If Not _IsCustomerServed Then
                            txtXDvdNotFound.EditValue += 1
                            txtCustomersNotServed.EditValue += 1
                            _custRow.Item("NotServed") = 1
                            _custRow.Item("NoDVDFound") = 1
                        Else
                            'If _custRow.Item("Served") = 0 Then
                            txtCustomersServed.EditValue += 1
                            _custRow.Item("Served") = 1
                            'End If
                        End If
                    End If
                Else 'End If _CustInNeed = YES

                End If 'Is _CustInNeed = NO
                txtCustomersToProcess.EditValue -= 1
                DoEvents()
            Next
            If _AtLeastOneHasBeenServed = False Then
                Exit Do
            End If
            _loop += 1
        Loop
