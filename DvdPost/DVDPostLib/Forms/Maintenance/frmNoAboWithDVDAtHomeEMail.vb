    ...
    Private Sub SendMails()
        ...
        For Each _custrow In ViewCustToSend '_i = 0 To GridView1.RowCount - 1
            ...
            If ViewMessage.Count = 1 Then
                'Loading Orders with Status = 2 - Expedited but never returned
                sSQL4 = "SELECT o.orders_id, op.products_id, pd.products_name FROM orders o " & _
                " left join orders_products op on op.orders_id = o.orders_id left join products_description pd on " & _
                " pd.products_id = op.products_id and pd.language_id = " & _custrow.Item("customers_language") & _
                " where o.orders_status = 2 and o.customers_id = " & _custrow.Item("customers_id")
                _DS.Tables("TmpTable").Clear()
                BKGlobal.Cls1.FillDataSet(_DS.Tables("TmpTable"), sSQL4)
                For Each _ProductRow In _DS.Tables("TmpTable").Rows
                    ...
                    strMessage = Replace(strMessage, "$$$products_name$$$", _ProductRow.Item("products_name") & "")
