...
Public Class clsCoda2
    ...
    Private Function LoadStruct(ByVal lstCodaType As List(Of Coda), ByVal filePath As String, ByRef index_EndVirement As Integer) As List(Of Coda)
        ...
        For Each line As String In struct
            ...
            Else
                'clsMsgError.MsgBox("error in file " & filePath & " " & line)
                lstDataCoda.Clear()
                Return lstDataCoda
            End If
            cpt_line += 1
    ...
    Private Sub updateDomUnpaid(ByVal coda_filename As String) ', ByVal lst_datePivot As String)
        ...
        Try
            ' flag the no payment of coda's today 
            ...
            '' flag status payment the insolvent dom of coda's today definitif status
            'sql = DvdPostData.ClsBatchDomiciliation.getChangeStatusUnpaidInsolvent()
            'DvdPostData.clsConnection.ExecuteNonQuery(sql)

            ' flag status paid if the status is waiting_payment and date_added + 15 days = today  
        ...
        Catch ex As Exception
            ...
            MsgBox("Error Matching Domiciliation " & ex.Message, MsgBoxStyle.Critical)
        End Try
