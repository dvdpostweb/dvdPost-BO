...
#End Region
    ...
    ByVal TotalNumberFund As Integer _
        ...
        strFooter = strFooter + formatLeft(totalPaymentDomID, LEN_TOTAL_PAYMENTDOMID) + formatLeft(numberofRefund, LEN_CPT_REFUND)
        strFooter = strFooter + formatLeft(TotalRefundAmount, LEN_TOTAL_AMOUNT_REFUND) + formatLeft(TotalNumberFund, LEN_TOTAL_REFUNDDOMID)
        strFooter = strFooter + endStr + vbLf
        Return strFooter
    End Function

    Private Function getDomiciliation(ByVal drReconduction As DataRow) As String
        If drReconduction("domiciliation_number") Is DBNull.Value Then
            Return "0"
        Else
    ...
    Private Function LineData(ByVal drReconduction As DataRow, ByVal numsequence As Integer, ByVal strcommunication As String) As String
        ...
        Try
            ...
            '    ' Dim cls As ClsCustomers = New ClsCustomers()
            '    clsMsgError.MsgBox("customers " & cls.GetCustomersId(drReconduction) & " domiciliation_number is null")
            '    Return ""
            'End If
