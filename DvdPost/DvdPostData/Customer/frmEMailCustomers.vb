...
" order by mmsh.cpt desc "
    ...
    Public Shared Function GetDVDNOTReturnedMoreThenMonth()
        Dim sql As String

        sql = "SELECT c.customers_id, " & clsMassEmail.CapitalizeSql("customers_firstname", "customers_lastname") & " as customers_name, " & _
                " c.customers_email_address, c.customers_language " & _
                "FROM orders o " & _
                "join orders_products op on o.orders_id = op.orders_id " & _
                "join customers c on o.customers_id = c.customers_id " & _
                "where date(o.date_purchased) = date(date_add(now(),interval -30 day)) and o.orders_status <> 3 " & _
                "and not exists (select * from orders o1 where o1.customers_id = o.customers_id and o1.date_purchased > o.date_purchased ) "

        Return sql

    End Function

    Public Shared Function GetNewsletter20150930()
        Dim sql As String

        sql = "SELECT customers_id, " & clsMassEmail.CapitalizeSql("customers_firstname", "customers_lastname") & " as customers_name, " & _
                " customers_email_address, customers_language " & _
                "FROM customers " & _
                "where  customers_abo = 0 and customers_id = 1068898"

        Return sql

    End Function

    Public Shared Function SendVirmanAnnouncement()
        Dim sql As String

        sql = "SELECT customers_id, " & clsMassEmail.CapitalizeSql("customers_firstname", "customers_lastname") & " as customers_name, " & _
                " customers_email_address, customers_language " & _
                "FROM customers " & _
                "where customers_id = 1068898"
